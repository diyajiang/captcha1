...
// 要点击的词语
let arr=['一往无前','二龙腾飞','三思而行','四季平安','五福临门'];
// 混淆的字
...
function start(){
    ...
    // 初始化titbox
    titbox.innerHTML='';
    titbox.style.color=' rgb(171, 128, 202)';
    titbox.style.opacity=0.6;

    // 获取要点击的词语
    let math=arr[Math.floor(Math.random() * arr.length)];
    titbox.innerHTML=`依次点击: <span>${math}</span> 完成验证`;
    ...
    box.addEventListener('click',function(event){
        ...
        rad.innerText=a;
        rad.style.left=event.clientX - box.offsetLeft -240 + 'px';
        rad.style.top=event.clientY - box.offsetTop -340 + 'px';
        box.appendChild(rad);
        // 小绿点 结束
        // 共四个字
        if(a==4){
