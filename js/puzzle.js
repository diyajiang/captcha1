(function(win){
    ...
    function createCanvas(width,height){
        const canvas=createElement('canvas');//创建一个canvas元素节点
        canvas.width=width;
    ...
    function getRandomImg(){
        // 这个网站可以生成随机图片
        return 'https://picsum.photos/300/150/?image='+getRandomNumberByRange(0,100);//指定不同的图片编号。
    }

    // 创建图片
    function createImg(onload){
        const img=createElement('img');
        img.crossOrigin='Anonymous';//用于解决跨域访问图片的问题。
        img.onload=onload;
        img.onerror=()=>{//在图片加载错误时重新加载一张随机图片。
            img.src=getRandomImg();
        }
        img.src=getRandomImg();//通过调用getRandomImg()函数获取一个随机图片的URL，并将其赋值给img元素节点的src属性，触发图片加载。
        return img;
    }
    ...
    class captcha{
        ...
        initDOM(){
            ...
            const el=this.el;//获取传入的挂载元素el，并按照正确的层次顺序将这些元素添加到el中。
            ...
            Object.assign(this,{//将创建的各个元素和对应的上下文对象保存在this对象中，方便后续使用。
                canvas,
                block,
                sliderContainer,
                refreshIcon,
